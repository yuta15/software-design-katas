...
class OrderRepository(OrderRepositoryPort):
    ...
    def save(self, order: Order) -> None:
        ...
        if not self._exists(order.id):
            ...
            for order_item in order.order_items:
                items.append(
                    db_order_items(
                        order_id=order.id, item_name=order_item.item_name.value, item_price=order_item.item_price.value
                    )
                )
            ...
            self._session.exec(delete(db_order_items).where(col(db_order_items.order_id) == order.id))

            append_items = []
            for item in order.order_items:
                append_items.append(
                    db_order_items(order_id=order.id, item_name=item.item_name.value, item_price=item.item_price.value)
                )
    ...
    def _exists(self, order_id: UUID) -> bool:
        statement = select(exists().where(col(db_orders.id) == order_id))
        return self._session.exec(statement).one()

    def _get_db_orders(self, order_id: UUID) -> db_orders:
        db_order = self._session.get(db_orders, order_id)
        if db_order is None:
            raise LookupError(f"Order not found: {order_id}")
        return db_order

    def _get_db_items(self, order_id: UUID) -> Sequence[db_order_items]:
        return self._session.exec(select(db_order_items).where(col(db_order_items.order_id) == order_id)).all()
    ...
    def _build_order(self, db_order: db_orders, db_order_items: Sequence[db_order_items]) -> Order:
        order_items = []
        ...
        return Order.reconstruct(
            id=db_order.id, order_items=order_items, order_status=self._resolve_order_status(db_order.order_status)
        )
